...
static constexpr const int width_ = 120;
static constexpr const int height_ = 120;
...
int doSession(cv::Mat img,float * outinfo, std::mutex& msessionMutex)
{
	...
	clock_t t1 = clock();
		...
		session_->Run(Ort::RunOptions{ nullptr }, input_names, input_tensor_1, 1, output_names, &output_tensor_, 1);
		...
		if (gbpram1)
		{
			for (int k = 0; k < 62; ++k)
			{
				outRes1[k] = (outRes1[k]  + gOParam1[k]) / 2;
			}
			
		}
		gbpram1 = true;
		memcpy(gOParam1, outRes1, 62 * 4);
		
		for (int k = 0; k < 62; ++k)
		{
			resnew[k] = paramstd[k] * outRes1[k] + parammean[k];
		}
		//进行赋值操作
		...
		cv::Mat_<double> matin(3, 3);
		for (int k = 0; k < 9; ++k)
		{
			((double*)matin.data)[k] = fout[k];
		}
	    cv::Vec3f outpos = 	rotationMatrixToEulerAngles(matin);
	...
	outinfo[38365 * 3 + 0] = outpos[0];
	outinfo[38365 * 3 + 1] = outpos[1];
	outinfo[38365 * 3 + 2] = outpos[2];
