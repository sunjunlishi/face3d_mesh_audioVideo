	...
	while(hr = pEm->Next(1, &pM, &cFetched), hr==S_OK) {
		...
		if(SUCCEEDED(hr)) {
			...
			if (hr == NOERROR) {
				...
				::PostMessage(hList, CB_ADDSTRING, 0,(LPARAM)var.bstrVal);
					...
					if (pmt->cbFormat >= sizeof(VIDEOINFOHEADER))
					{
						...
						if(bMax)
						{
							...
							if( (bmiHeader->biWidth == nMaxWidth) &&
								(bmiHeader->biHeight == nMaxHeight) &&
								bmiHeader->biBitCount == 24 || bmiHeader->biBitCount == 16)
							{
								hr = pConfig->SetFormat(pmt);
								if(SUCCEEDED(hr))
								{
...
HRESULT CCaptureVideo::OpenDevices(int iDeviceID, HWND hWnd, int nWidth, int nHeight)
{
	...
	SetCaptureFormat(m_pCapture,m_pBF, nWidth, nHeight,true);
