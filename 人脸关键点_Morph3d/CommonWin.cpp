...

{
	...
	std::wstring strUnicode = Utf8ToWString(strAnsi.c_str());

	return wstringToString(strUnicode);
...
void DrawMyTextBK(CString text,int posX,int posY,CWnd * pWnd,COLORREF bkColor,int FontSize,COLORREF textColor)
{
	...
	if( pDc)
	{
		...
		shortrect.right = shortrect.left + text.GetLength()*FontSize*0.5;
...
static int  _nowLen = 0;
void ParseBuf(char * buf, int len, std::vector<std::string>& outinfo)
{
	if (_buf == NULL) _buf = new char[40000];
	memcpy(_buf + _nowLen, buf, len);
	_nowLen += len;
	if (len == 1024)
	{
		_buf[_nowLen] = '\0';
	...
	while (tokenPtr != NULL) {
		...
		if (tokenPtr)
		{
			...
			outinfo.push_back(lasts);
			if (strcmp(lasts, "11789 11661 0") == 0)
			{
				int kk = 0;
			}
		}
	...
	{
		//printf("last: %s", lasts);
		_nowLen = strlen(lasts);
		memcpy(_buf, lasts, strlen(lasts));
	}
	else
	{
		_nowLen = 0;
...
void filereadparse(char * filename, std::vector<std::string>& outinfo)
{
	...
	{
		char szBuf[1024];
		int len = fread(szBuf, 1, 1024, fp);
		while (len > 0)
		{
			ParseBuf(szBuf, len, outinfo);
			memset(szBuf, 0, sizeof(szBuf));
			len = fread(szBuf, 1, 1024, fp);
		}
