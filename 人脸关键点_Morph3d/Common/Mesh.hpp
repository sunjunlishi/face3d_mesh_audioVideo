...
struct Mesh
{
    std::vector<Vector3f> vertices;  ///< 3D vertex positions.
    std::vector<Vector3f> colors;    ///< Colour information for each vertex. Expected to be in RGB order.
    std::vector<Vector2f> texcoords; ///< Texture coordinates for each vertex.
...
inline void write_obj(Mesh mesh, std::string filename)
{
    ...
    if (mesh.colors.empty())
    {
        ...
        {
            obj_file << "v " << mesh.vertices[i].x << " " << mesh.vertices[i].y << " "
    ...
    } else
    {
        ...
        {
            obj_file << "v " << mesh.vertices[i].x << " " << mesh.vertices[i].y << " "
                     << mesh.vertices[i].z << " " << mesh.colors[i].x << " " << mesh.colors[i].y << " "
                     << mesh.colors[i].z << std::endl;
        }
    }
    ...
    if (!mesh.texcoords.empty())
    {
        ...
        {
            obj_file << "vt " << tc.x << " " << tc.y << std::endl;
        }
    }
...
inline void write_textured_obj(Mesh mesh, std::string filename)
{
    ...
    {
        obj_file << "vt " << mesh.texcoords[i].x << " " << 1.0f - mesh.texcoords[i].y << std::endl;
    ...
    texture_filename.replace(std::end(texture_filename) - 4, std::end(texture_filename), ".isomap.bmp");
