...
public:
    ...
    std::string c_str()
    {
        ...
        return format("{start:%.4f,end:%.4f}", start*1.0/260608, end*1.0 / 260608);
    };
private:
...
int testVad()
{
    ...
    // Read wav
    wav::WavReader wav_reader("outinfo5.wav"); //16000,1,32float
    ...
    // ===== Test configs =====
    std::wstring path = L"detection/silero_vad.onnx";
	...
	FILE * fp = fopen("D:/out.txt","wb");
    stamps = vad.get_speech_timestamps();
    for (int i = 0; i < stamps.size(); i++) {

        std::cout << stamps[i].c_str() << std::endl;
		std::string tmp = stamps[i].c_str() + "\n";
